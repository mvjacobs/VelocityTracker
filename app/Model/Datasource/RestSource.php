...
class RestSource extends DataSource {
    ...
    public function create(Model $model, $fields = null, $values = null) {
        $response = $this->Http->post(array(
            ...
            json_encode($model->data[$model->alias]),
            array(
                'header' => array(
                    'X-TrackerToken' => CakeSession::read('token'),
    ...
    public function read($model, $queryData = array()) {
        ...
        switch ($this->config['content-type']) {
    ...
    public function update($model, $fields = null, $values = null) {
        $response = $this->Http->put(array(
            'host' => $this->config['host'],
            'path' => $model->path,
            'port' => $this->config['port']),
            json_encode($model->data[$model->alias],JSON_FORCE_OBJECT),
            array(
                'header' => array(
                    'X-TrackerToken' => CakeSession::read('token'),
                    'Content-Type'   => $this->config['content-type'])));
     ...
     */
    public function request($model, $debug = false) {
        if (is_object($model)) {
        ...
        if(is_array($request['uri']) && (isset($model->path) || !empty($model->path))) {
            $request['uri']['path'] = $model->path;
        }

        if(is_array($request['uri']) && (!isset($request['uri']['host']) || empty($request['uri']['host']))) {
            $request['uri']['host'] = $this->config['host'];
        }

        if(is_array($request['uri']) && (!isset($request['uri']['port']) || empty($request['uri']['port']))) {
